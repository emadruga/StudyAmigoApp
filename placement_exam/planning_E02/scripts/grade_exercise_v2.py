...
# ─────────────────────────────────────────────────────────────────────────────
# Account map (secondary username → primary roster name)
# ─────────────────────────────────────────────────────────────────────────────

def load_account_map(path: Path) -> Dict[str, str]:
    """
    Parse the account-map CSV.

    Expected columns (case-insensitive):
        username, roster_name

    Returns {username_lower: roster_name} for every row.
    """
    if not path.exists():
        sys.exit(f"Account map not found: {path}")
...
def apply_account_map(
    ...
    """
    For each (username → roster_name) entry in account_map, find the admin
    user_id matching that username and append it to the _matched_uids of the
    corresponding roster student.  Must be called AFTER match_roster_to_admin.
    """
    # Build username (lower) → uid
    username_to_uid: Dict[str, int] = {
        info["username"].lower(): uid for uid, info in users.items()
    }
    # Build roster_name (normalised) → roster index
    roster_norm_to_idx: Dict[str, int] = {
        _normalise_name(s["name"]): i for i, s in enumerate(roster)
    }

    for username_lower, roster_name in account_map.items():
        uid = username_to_uid.get(username_lower)
        if uid is None:
            print(f"  [account_map] aviso: username '{username_lower}' não encontrado em admin.db — ignorado")
            continue

        target_norm = _normalise_name(roster_name)
        idx = roster_norm_to_idx.get(target_norm)
        if idx is None:
            print(f"  [account_map] aviso: roster_name '{roster_name}' não encontrado no roster — ignorado")
            continue

        student = roster[idx]
        # Ensure the student has a primary uid (may not have been matched by fuzzy)
...
def load_roster(roster_path: Path) -> List[Dict]:
    ...
    Expected columns (case-insensitive, Portuguese aliases accepted):
        Course / Curso, ID, Name / Nome, Email, Path / Caminho, Suggested Tier
    ...
    with open(roster_path, newline="", encoding="utf-8-sig") as f:
        ...
        for row in reader:
            norm = {k.strip().lower(): v.strip() for k, v in row.items()}
            students.append({
                "course":     norm.get("course", norm.get("curso", "")),
                "student_id": norm.get("id", ""),
                "name":       norm.get("name", norm.get("nome", "")),
                "email":      norm.get("email", ""),
                "path":       norm.get("path", norm.get("caminho", "")),
...
def main():
    ...
    parser.add_argument(
        "--account-map", default=None, metavar="FILE",
        help="CSV mapping secondary usernames to roster names (columns: username, roster_name). "
             "Applied after fuzzy matching to attach duplicate accounts to their primary student.",
    )
    ...
    if args.roster:
        ...
        if args.account_map:
            account_map = load_account_map(Path(args.account_map))
            apply_account_map(roster, users, account_map)
            print(f"  {len(account_map)} mapeamento(s) de conta carregado(s)  ({args.account_map})")
