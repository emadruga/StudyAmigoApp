#!/bin/bash
# =============================================================================
# StudyAmigo v1.5 - Backup Container Entrypoint
# =============================================================================
...
#   3. Once the bucket is reachable, sleep until the next 08:00 UTC then
#      run the backup. Loop forever.
#
# Schedule : 08:00 UTC daily = 05:00 America/Sao_Paulo (BRT = UTC-3, no DST)
#            (2 hours after SAv1.0 backup at 06:00 UTC — no S3 contention)
#
# S3 prefix: backups/v15/week-{1..4}/{day-of-week}/
#            Separate from SAv1.0 (backups/week-{1..4}/{day-of-week}/)
#
...
BACKUP_PREFIX="${BACKUP_PREFIX:-backups/v15}"
...
run_backup() {
  ...
  s3_prefix="s3://${bucket}/${BACKUP_PREFIX}/week-${slot}/${dow}"

  log "=== Backup starting: ${BACKUP_PREFIX}/week-${slot}/${dow} → ${s3_prefix} ==="
  ...
  cat > "${tmp}/meta.json" <<EOF
{
  ...
  "s3_prefix":                "${s3_prefix}",
  "source":                   "docker-compose-backup-sidecar-v15"
}
EOF
...
# Main loop
# ---------------------------------------------------------------------------
log "Backup sidecar started (project: ${PROJECT_NAME}, prefix: ${BACKUP_PREFIX})"
...
while true; do
  ...
  if ! bucket_reachable "$BUCKET"; then
    log "WARNING: Bucket '${BUCKET}' is not reachable. Retrying in 1 h."
    sleep 3600
    BUCKET=""
    continue
  fi

  # Step 3 — sleep until next 08:00 UTC
  secs=$(secs_until_utc 8 0)
